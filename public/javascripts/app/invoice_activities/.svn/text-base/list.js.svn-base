	...
	addEditListeners: function(){
		...
		$('td.activity-edit').live('click', function() {
			if(!$.inEditMode()){
				...
				$('#activity_edit_link').attr('href', '/invoice_activities/'+id+'/edit');
		...
		if(undefined != data){
			...
			if(undefined != response.success && response.success == false){
				...
				for(var prop in errors) {
					// find the form input
					if(prop == 'project_id'){
						var inputs = $(id).find("input[name='activity_project_number']")
					}else{
...
$(document).ready(function() {
	...
	$('#activity_edit_link').bind('ajax:loading', function(xhr){
		var id = $(this).attr('href').replace('/invoice_activities/', '#load_edit_');
		id = id.replace('/edit','');
		...
		if(!$.inEditMode()){
			...
			if(n > 0){
				if(confirm('Delete '+n+' items? This cannot be undone!')){
					...
					$.ajax({
	                	url: '/invoice_activities/list_action',
	                    data: data,
	                    dataType: 'script',
	                    type: 'POST',
