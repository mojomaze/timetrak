class User < ActiveRecord::Base
  ...
  def self.search(query, page, sort_column, sort_direction)
    if query
      search = "%#{query}%"
      return order(sort_column+" "+sort_direction).where("username LIKE ? OR full_name LIKE ? OR billing_name LIKE ?", search, search, search).paginate(:page => page, :per_page => self.per_page)
   end
